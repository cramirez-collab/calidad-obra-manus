...
export default function EditorPruebas() {
  ...
  const { data: catalogo, isLoading, refetch } = trpc.pruebas.catalogoAll.useQuery(
      ...
      <div className="max-w-4xl mx-auto px-3 py-4 sm:px-6 sm:py-6">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="text-xl font-bold text-[#002C63] truncate">Editor de Pruebas</h1>
            <p className="text-sm text-muted-foreground">
              {activePruebas} activas de {totalPruebas} · {sistemas.length} sistemas
            </p>
          </div>
          <Button
            size="sm"
          ...
          <div className="space-y-2">
            {sistemas.length === 0 ? (
              <div className="text-center py-12 text-muted-foreground">
                ...
                <p className="text-sm">Agrega pruebas con el botón "Nueva".</p>
              </div>
            ) : (
              sistemas.map((sistema) => {
                const pruebas = grouped.get(sistema) || [];
                const isExpanded = expandedSistemas.has(sistema);
                const activas = pruebas.filter((p: any) => p.activo).length;

                return (
                  <div key={sistema} className="bg-white border border-gray-100 rounded-xl overflow-hidden">
                    {/* System header */}
                    <button
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <span className="font-semibold text-[#002C63]">{sistema}</span>
                        <span className="text-xs text-muted-foreground ml-2">
                          {activas} prueba{activas !== 1 ? "s" : ""} activa{activas !== 1 ? "s" : ""}
                        </span>
                      </div>
                      <Button
                        variant="ghost"
                        ...
                        {pruebas.map((prueba: any) => (
                          <div
                            ...
                            className={`flex items-center gap-3 px-4 py-2.5 border-b border-gray-50 last:border-0 ${
                              !prueba.activo ? "opacity-50 bg-gray-50" : ""
                            }`}
                          >
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2">
                                <span className={`text-sm font-medium ${!prueba.activo ? "line-through text-muted-foreground" : "text-foreground"}`}>
                                  {prueba.nombre}
                                </span>
                                {!prueba.activo && (
                              ...
                              {prueba.descripcion && (
                                <p className="text-xs text-muted-foreground mt-0.5 truncate">{prueba.descripcion}</p>
                              )}
                            </div>
                            <div className="flex items-center gap-1 shrink-0">
                              ...
                              {prueba.activo ? (
                                <Button
                                  ...
                                  size="icon"
                                  className="h-7 w-7"
                                  onClick={() => eliminarMut.mutate({ id: prueba.id })}
                                ...
                                <Button
                                  ...
                                  size="icon"
                                  className="h-7 w-7"
                                  onClick={() => reactivarMut.mutate({ id: prueba.id })}
      ...
      <Dialog open={createModal} onOpenChange={setCreateModal}>
        <DialogContent className="max-w-md">
            ...
            <div className="space-y-2">
              ...
              <Select value={formSistema} onValueChange={setFormSistema}>
                ...
                <SelectContent>
                  ...
                  {/* Custom systems from existing catalog */}
                  {sistemas.filter(s => !SISTEMAS_DEFAULT.includes(s)).map((s) => (
                    <SelectItem key={s} value={s}>{s}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
          ...
          <DialogFooter>
            ...
            >
              {crearMut.isPending ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Plus className="w-4 h-4 mr-2" />}
              Crear
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={!!editModal} onOpenChange={(open) => !open && setEditModal(null)}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            >
              {actualizarMut.isPending ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Pencil className="w-4 h-4 mr-2" />}
              Guardar
            </Button>
          </DialogFooter>
        </DialogContent>
